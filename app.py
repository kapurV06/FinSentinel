...
with st.sidebar:
    ...
    api_key = st.text_input("Anthropic API Key", type="password",
                            placeholder="sk-ant-...")
    st.caption("Required for AI Agent tab only")
    st.markdown("---")
...
if show_agent:
    ...
    if not api_key:
        st.info("⬅ Enter your Anthropic API key in the sidebar to enable the AI Agent.")
    else:
        import anthropic
...
Answer concisely. Always call tools before answering. Use specific numbers. Be direct — no fluff."""
                        ...
                        if resp.stop_reason == "tool_use":
                            ...
                            for blk in resp.content:
                                if blk.type == "tool_use":
                                    res = run_tool(blk.name, blk.input)
                                    results.append({
                                        "type": "tool_result",
                                        "tool_use_id": blk.id,
